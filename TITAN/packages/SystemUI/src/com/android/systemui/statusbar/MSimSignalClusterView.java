...
public class MSimSignalClusterView
    ...
    private ImageView mMobileCdma3g, mMobileCdma1x, mMobileCdma1xOnly, mMobileUpSignalView, mMobileTypeDownView;
    ...
    private boolean[] mMSimDataConnected;
    ...
    private int[] mMobileTypeRoamingResourceId = {R.id.mobile_type_roaming, R.id.mobile_type_roaming_sub2,
            R.id.mobile_type_roaming_sub3};
    ...
    public MSimSignalClusterView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mMobileTypeRoaming = new ImageView[mNumPhones];
    ...
    protected void onAttachedToWindow() {
        ...
        for (int i = 0; i < mNumPhones; i++) {
            ...
            mMobileTypeRoaming[i] = (ImageView) findViewById(mMobileTypeRoamingResourceId[i]);
        ...
        mMobileUpSignalView = (ImageView) findViewById(R.id.mobile_signal_up);
        mMobileTypeDownView = (ImageView) findViewById(R.id.mobile_type_down);
    ...
    public void setMobileDataIndicators(boolean visible, int strengthIcon, int activityIcon,
        ...
        mRoaming = isRoaming;
        mMSimDataConnected = dataConnected;
        /*if(phoneId == 0) {
            Log.v("fangjunran", "mMSimDataConnected 0 = "+mMSimDataConnected[0]);
            Log.v("fangjunran", "getLteLevel = "+signalStrength.getLteLevel());
            Log.v("fangjunran", "getAlternateLteLevel = "+signalStrength.getAlternateLteLevel());
            Log.v("fangjunran", "getCdmaLevel = "+signalStrength.getCdmaLevel());
            Log.v("fangjunran", "getEvdoLevel = "+signalStrength.getEvdoLevel());
            Log.v("fangjunran", "@@@@getLteRsrp = "+signalStrength.getLteRsrp());
            Log.v("fangjunran", "@@@@getCdmaDbm = "+signalStrength.getCdmaDbm());
            Log.v("fangjunran", "@@@@getEvdoDbm = "+signalStrength.getEvdoDbm());
        } else {
            Log.v("fangjunran", "mMSimDataConnected 1 = "+mMSimDataConnected[1]);
            Log.v("fangjunran", "getGsmLevel = "+signalStrength.getGsmLevel());
        }*/
        ...
        if (mStyle == STATUS_BAR_STYLE_CDMA_1X_COMBINED) {
            ...
            mMobileDataVoiceVisible[phoneId] = false;
/*            Log.v("fangjunran", "sub : "+phoneId+" typeIcon name = "+getResourceName(typeIcon));
            Log.v("fangjunran", "sub : "+phoneId+" activityIcon name = "+getResourceName(activityIcon));
            Log.v("fangjunran", "sub : "+phoneId+" strengthIcon name = "+getResourceName(strengthIcon));
            Slog.d("fangjunran", "##getLteLevel = "+signalStrength.getLteLevel());
            Slog.d("fangjunran", "##getAlternateLteLevel = "+signalStrength.getAlternateLteLevel());
            Slog.d("fangjunran", "##getCdmaLevel = "+signalStrength.getCdmaLevel());
            Slog.d("fangjunran", "##getEvdoLevel = "+signalStrength.getEvdoLevel());*/
            if (phoneId == 0) {
                if (mRoaming || strengthIcon == R.drawable.stat_sys_signal_null) {
//                    Slog.d("fangjunran", "##roaming or null signal.");
                    mMobileStrengthId[phoneId] = strengthIcon;
                    mMobileTypeId[phoneId] = typeIcon;
                    mMobileActivityId[phoneId] = activityIcon;
                    if(mMobileUpSignalView != null) {
                        mMobileUpSignalView.setVisibility(View.GONE);
                        mMobileTypeDownView.setVisibility(View.GONE);
                    }
                } else if(signalStrength.getLteLevel() != 0 || signalStrength.getEvdoLevel() != 0) {
                    if (typeIcon == R.drawable.stat_sys_data_fully_connected_3g
                            || typeIcon == R.drawable.stat_sys_data_connected_3g) {
//                        Slog.d("fangjunran", "##3g signal.");
//                        Slog.d("fangjunran", "##mMobileUpSignalView == null ? "+(mMobileUpSignalView == null));
                        if(mMobileUpSignalView != null) {
                            mMobileUpSignalView.setImageResource(TELEPHONY_SIGNAL_STRENGTH_3G_4G[signalStrength.getEvdoLevel()]);
                            mMobileUpSignalView.setVisibility(View.VISIBLE);
                        }
                        mMobileActivityId[phoneId] = getDataTypeIcon(activityIcon);
                        mMobileTypeId[phoneId] = R.drawable.stat_dualdata_up_3g;
                        if(mMobileTypeDownView != null) {
                            mMobileTypeDownView.setImageResource(R.drawable.stat_dualdata_down_2g);
                            mMobileTypeDownView.setVisibility(!mWifiVisible ? View.VISIBLE : View.GONE);
                        }
                    } else if (typeIcon == R.drawable.stat_sys_data_fully_connected_4g
                            || typeIcon == R.drawable.stat_sys_data_connected_4g) {
//                        Slog.d("fangjunran", "##4g signal.");
                        if(mMobileUpSignalView != null) {
                            mMobileUpSignalView.setVisibility(View.VISIBLE);
                            mMobileUpSignalView.setImageResource(TELEPHONY_SIGNAL_STRENGTH_3G_4G[signalStrength.getLteLevel()]);
                        }
                        mMobileActivityId[phoneId] = getDataTypeIcon(activityIcon);
                        mMobileTypeId[phoneId] = R.drawable.stat_dualdata_up_4g;
                        if(mMobileTypeDownView != null) {
                            mMobileTypeDownView.setImageResource(R.drawable.stat_dualdata_down_2g);
                            mMobileTypeDownView.setVisibility(!mWifiVisible ? View.VISIBLE : View.GONE);
                        }
                    } else if (typeIcon == 0) {
//                        Slog.d("fangjunran", "##no data icon.");
                        if (signalStrength.getLteLevel() != 0) {
//                            Slog.d("fangjunran", "##no data icon. 3g");
                            if(mMobileUpSignalView != null) {
                                mMobileUpSignalView.setVisibility(View.VISIBLE);
                                mMobileTypeDownView.setVisibility(View.GONE);
                                mMobileUpSignalView.setImageResource(TELEPHONY_SIGNAL_STRENGTH_3G_4G[signalStrength.getLteLevel()]);
...
//                            Slog.d("fangjunran", "##no data icon. 4g");
                            if(mMobileUpSignalView != null) {
                                mMobileUpSignalView.setVisibility(View.VISIBLE);
                                mMobileTypeDownView.setVisibility(View.GONE);
                                mMobileUpSignalView.setImageResource(TELEPHONY_SIGNAL_STRENGTH_3G_4G[signalStrength.getEvdoLevel()]);
                    ...
                    mMobileStrengthId[phoneId] = TELEPHONY_SIGNAL_STRENGTH_2G[signalStrength.getCdmaLevel()];
                } else {
//                    Slog.d("fangjunran", "##single signal icon.");
                    mMobileStrengthId[phoneId] = strengthIcon;
                    mMobileTypeId[phoneId] = typeIcon;
                    mMobileActivityId[phoneId] = activityIcon;
                    if(mMobileUpSignalView != null) {
    ...
    private void apply(int phoneId) {
        ...
        if (mStyle == STATUS_BAR_STYLE_ANDROID_DEFAULT) {
            ...
            mMobileTypeRoaming[phoneId].setVisibility(mRoaming ? View.VISIBLE : View.GONE);
        } else {
            mMobileType[phoneId].setVisibility(View.GONE);
            mMobileTypeRoaming[phoneId].setVisibility(View.GONE);
        }
...
//        if (mStyle != STATUS_BAR_STYLE_ANDROID_DEFAULT) {
            if (mNoSimIconId[phoneId] != 0) {
                ...
                if (phoneId == 0 && mMobileUpSignalView != null) {
                    mMobileUpSignalView.setVisibility(GONE);
                }
            } else {
...
//        }
    ...
    private void updateMobile(int phoneId) {
        ...
        mMobileTypeRoaming[phoneId].setImageResource(R.drawable.stat_sys_data_fully_connected_2g);
    ...
    static final int[] TELEPHONY_SIGNAL_STRENGTH_2G = {
            R.drawable.stat_dualsignal_down0,
            R.drawable.stat_dualsignal_down1,
            R.drawable.stat_dualsignal_down2,
            R.drawable.stat_dualsignal_down3,
            R.drawable.stat_dualsignal_down4 };

    static final int[] TELEPHONY_SIGNAL_STRENGTH_3G_4G = {
        R.drawable.stat_dualsignal_up0,
        R.drawable.stat_dualsignal_up1,
        R.drawable.stat_dualsignal_up2,
        R.drawable.stat_dualsignal_up3,
        R.drawable.stat_dualsignal_up4 };

    private int getDataTypeIcon (int icon) {
        int returnVal = 0;
        switch(icon){
            case R.drawable.stat_sys_signal_inout_auto_mirrored:
            case R.drawable.stat_sys_signal_inout:
                returnVal = R.drawable.stat_sys_dualsignal_inout;
                break;
            case R.drawable.stat_sys_signal_in_auto_mirrored:
            case R.drawable.stat_sys_signal_in:
                returnVal = R.drawable.stat_sys_dualsignal_in;
                break;
            case R.drawable.stat_sys_signal_out_auto_mirrored:
            case R.drawable.stat_sys_signal_out:
                returnVal = R.drawable.stat_sys_dualsignal_out;
                break;
            default:
                break;
        }
