...
public class PersistentDataBlockManager {
    ...
    public int openPartition() {
        try {
            return sService.openPartition();
        } catch (RemoteException e) {
            onError("open Partition");
            return -1;
        }
    }  
    /**
    ...
    public void closepartition(){
    	try {
            sService.closepartition();
        } catch (RemoteException e) {
            onError("close Partition");
        }
    }
    /**
     * @hide
    ...
    public int writepartition(int offset, int size, String readbuf){
    	try {
           return sService.writepartition(offset,size,readbuf);
        } catch (RemoteException e) {
            onError("write Partition");
            return -1;
        }
    }
     ...
     * @hide
     */
    public int readpartition(int offset, int size, String readbuf){
    	try {
            return sService.readpartition(offset,size,readbuf);
        } catch (RemoteException e) {
            onError("read Partition");
            return -1;
        }
    }
    /**
    ...
    public int checkrootornot(){
    	try {
            return sService.checkrootornot();
        } catch (RemoteException e) {
            onError("read Partition");
            return -1;
        }
