...
class AlarmManagerService extends AbsAlarmManagerService {
    ...
    void rescheduleKernelAlarmsLocked() {
        ...
        if (mAlarmBatches.size() > 0) {
            ...
            if (firstRtcWakeup != null && mNextRtcWakeup != firstRtcWakeup.start) {
                ...
                if (when != 0) {
                    setLocked(RTC_POWEROFF_WAKEUP, when - POWER_OFF_ALARM_THRESHOLD);
                }
            }
    ...
    void deliverAlarmsLocked(ArrayList<Alarm> triggerList, long nowELAPSED) {
        ...
        for (int i=0; i<triggerList.size(); i++) {
            ...
            try {
                /*< DTS2014092903562 xufusheng/00297139 20141010 begin*/
                if ((AlarmManager.RTC_POWEROFF_WAKEUP == alarm.type)||(AlarmManager.RTC_WAKEUP == alarm.type) || (AlarmManager.ELAPSED_REALTIME_WAKEUP == alarm.type)){
                    Slog.v(TAG, "sending alarm " + alarm + ",repeatInterval = " + alarm.repeatInterval);
                }
                /* DTS2014092903562 xufusheng/00297139 20141010 end>*/
                /*< DTS2014092903562 xufusheng/00297139 20141010 begin*/
                if (IS_POWEROFF_ALARM_ENABLED) {
                    if (AlarmManager.RTC_POWEROFF_WAKEUP == alarm.type
                        && DESKCLOCK_PACKAGENAME.equals(alarm.operation.getTargetPackage())) {
                        if (true == mIsFirstPowerOffAlarm && "RTC".equals(SystemProperties.get("persist.sys.powerup_reason", "NORMAL"))) {
                            mBackgroundIntent.putExtra("FLAG_IS_FIRST_POWER_OFF_ALARM", true);
                            android.util.Log.d("ljj", "FLAG_IS_FIRST_POWER_OFF_ALARM  :true");
                            mIsFirstPowerOffAlarm = false;
                        } else {
                            mBackgroundIntent.putExtra("FLAG_IS_FIRST_POWER_OFF_ALARM", false);
                            android.util.Log.d("ljj", "FLAG_IS_FIRST_POWER_OFF_ALARM  :false");
                        }
                    }
                }
                /* DTS2014092903562 xufusheng/00297139 20141010 end>*/
