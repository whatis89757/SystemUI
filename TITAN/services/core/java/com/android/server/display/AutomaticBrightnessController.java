...
class AutomaticBrightnessController {
    ...
    private final Spline mScreenAutoBrightnessDarkenedSpline;
    private boolean mScreenAutoBrightnessDarkened;
    ...
    public AutomaticBrightnessController(Callbacks callbacks, Looper looper,
        ...
        this(callbacks, looper, sensorManager, autoBrightnessSpline, null, lightSensorWarmUpTime, brightnessMin, brightnessMax, dozeScaleFactor);
    }

    public AutomaticBrightnessController(Callbacks callbacks, Looper looper,
            SensorManager sensorManager, Spline autoBrightnessSpline, Spline autoBrightnessDarkenedSpline, int lightSensorWarmUpTime,
            int brightnessMin, int brightnessMax, float dozeScaleFactor) {
        mCallbacks = callbacks;
        mTwilight = LocalServices.getService(TwilightManager.class);
        mSensorManager = sensorManager;
        mScreenAutoBrightnessSpline = autoBrightnessSpline;
        mScreenAutoBrightnessDarkenedSpline = autoBrightnessDarkenedSpline;
        mScreenAutoBrightnessDarkened = false;
    ...
    private void updateAmbientLux(long time) {
        ...
        if (ambientLux >= mBrighteningLuxThreshold && nextBrightenTransition <= time
            ...
            mScreenAutoBrightnessDarkened = (ambientLux < mAmbientLux);
            if (DEBUG) {
    ...
    private void updateAutoBrightness(boolean sendUpdate) {
        ...
        float value =mScreenAutoBrightnessSpline.interpolateSimple(mAmbientLux);
        float gamma = 1.0f;

        if (mScreenAutoBrightnessDarkened && mScreenAutoBrightnessDarkenedSpline != null) {
            value =mScreenAutoBrightnessDarkenedSpline.interpolateSimple(mAmbientLux);
        }
        if (DEBUG) {
            Slog.d(TAG, "updateAutoBrightness: mScreenAutoBrightnessDarkened = " + mScreenAutoBrightnessDarkened 
                        + " mAmbientLux = " + mAmbientLux + " value = " + value + " mScreenAutoBrightnessSpline = " 
                        + mScreenAutoBrightnessSpline + " mScreenAutoBrightnessDarkenedSpline = " 
                        + mScreenAutoBrightnessDarkenedSpline);
        }
