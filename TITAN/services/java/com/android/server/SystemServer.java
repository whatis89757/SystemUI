...
import com.android.server.ram.DeviceRamMonitorService;
...
public final class SystemServer {
    ...
    private void startBootstrapServices() {
        ...
        // Only run "core" apps if we're recovery data.
        String dataRecovery = SystemProperties.get("persist.sys.datarecovery");
        if ("1".equals(dataRecovery)) {
            Slog.w(TAG, "Prepare data recovery - only parsing core apps");
            mOnlyCore = true;
            updateBootAbnormal();
        } 
    ...
    private void startOtherServices() {
        ...
        if (mFactoryTestMode != FactoryTest.FACTORY_TEST_LOW_LEVEL) {
            ...
            mSystemServiceManager.startService(DeviceRamMonitorService.class);
    ...
    private static final String BOOT_ABNORML_DAT = "persist/bootabnormal.dat";
    private void updateBootAbnormal () {
        File file = new File(BOOT_ABNORML_DAT);
        int count = 0;
        if (file.exists()) {
            FileInputStream fileInput = null;
            try {
                fileInput = new FileInputStream(file);
                count = fileInput.read();
            } catch (Exception e) {
                Log.d(TAG, "updateBootAbnormal: read fail " + e);
            } finally {
                if (fileInput != null) {
                    try {
                        fileInput.close();
                    } catch (Exception e) {
                        Log.d(TAG, "updateBootAbnormal: read close fail " + e);
                    } finally {
                        fileInput = null;
                    }
                }
        ...
        try {
            ...
            if (fileOutput != null) {
                try {
                    ...
                    Log.d(TAG, "updateBootAbnormal: write close fail " + e);
                } finally {
                    fileOutput = null;
                }
            }
