...
const ToneGenerator::ToneDescriptor ToneGenerator::sToneDescriptors[] = {
        ...
        { .segments = { { .duration = 400,   .waveFreq = { 400, 425}, 0, 0 },
                        { .duration = 200,   .waveFreq = { 0 }, 0, 0 },
                        { .duration = 400,   .waveFreq = { 400, 425}, 0, 0 },
                        { .duration = 2000,  .waveFreq = { 0 }, 0, 0 },
                        { .duration = 0 ,    .waveFreq = { 0 }, 0, 0}},
          .repeatCnt = ToneGenerator::TONEGEN_INF,
          .repeatSegment = 0 }, // TONE_INDIA_RINGBACK
};
...
const unsigned char /*tone_type*/ ToneGenerator::sToneMappingTable[NUM_REGIONS-1][NUM_SUP_TONES] = {
        ...
        { // INDIA
            ...
            TONE_SUP_ERROR,             // TONE_SUP_ERROR
            TONE_SUP_CALL_WAITING,      // TONE_SUP_CALL_WAITING
            TONE_INDIA_RINGBACK         // TONE_SUP_RINGTONE
        }      
};
...
ToneGenerator::ToneGenerator(audio_stream_type_t streamType, float volume, bool threadCanCallJava) {
    ...
    if (strstr(value, "us") != NULL ||
        ...
        ALOGD("Choosing ANSI standard tones");
        mRegion = ANSI;
    } else if (strstr(value, "jp") != NULL) {
        ALOGD("Choosing JAPAN standard tones");
        mRegion = JAPAN;
	} else if (strcmp(value,"in") == 0 || strcmp(value,",in")==0  || strcmp(value,"in,in")==0)  {
        ALOGD("Choosing INDIA standard tones");
        mRegion = INDIA;
        ...
        mRegion = CEPT;
		ALOGE("gsm.operator.iso-country %d", mRegion );
    }
