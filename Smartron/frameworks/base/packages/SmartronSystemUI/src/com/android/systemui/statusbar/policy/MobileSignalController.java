...
public class MobileSignalController extends SignalController<
    ...
    private void mapIconSets() {
        ...
        if (!mConfig.showAtLeast3G) {
            ...
            mNetworkToIconLookup.put(TelephonyManager.NETWORK_TYPE_1xRTT,
                    TelephonyIcons.H);
            mDefaultIcons = TelephonyIcons.H;
        }
    ...
    public void notifyListeners(SignalCallback callback) {
        ...
        if (callback instanceof SignalCallbackExtended) {
            ((SignalCallbackExtended)callback).setMobileDataIndicators(statusIcon, qsIcon, typeIcon,
                    ...
                    mSubscriptionInfo.getSubscriptionId(), getEmbmsIconId(),
                    isMobileIms(), isImsRegisteredInWifi(), mCurrentState.roaming);
        } else {
    ...
    private boolean isMobileIms() {
        ...
        if (subInfos != null) {
            for (SubscriptionInfo subInfo: subInfos) {
                int subId = subInfo.getSubscriptionId();
                if (mPhone != null) {
                    Log.d(mTag, "isMobileIms : subId = "+subId+", phoneId = "+subInfo.getSimSlotIndex()+", isImsRegisteredForSubscriber = "
                            +mPhone.isImsRegisteredForSubscriber(subId)+", isVolteAvailable = "
                            +mPhone.isVolteAvailable()+", isImsRegisteredInWifi = "+isImsRegisteredInWifi());
                    if (mPhone.isImsRegisteredForSubscriber(subId)
    ...
    private void updateDataSim() {
        ...
        if (SubscriptionManager.isValidSubscriptionId(defaultDataSub) && SubscriptionManager.from(mContext).isActiveSubId(defaultDataSub)) {
    ...
    private int getAlternateLteLevel(SignalStrength signalStrength) {
        ...
        else if (lteRsrp >= -99) rsrpLevel = SignalStrength.SIGNAL_STRENGTH_GREAT;
        else if (lteRsrp >= -115) rsrpLevel = SignalStrength.SIGNAL_STRENGTH_GOOD;
        else if (lteRsrp >= -120) rsrpLevel = SignalStrength.SIGNAL_STRENGTH_MODERATE;
        else if (lteRsrp >= -128) rsrpLevel = SignalStrength.SIGNAL_STRENGTH_POOR;
