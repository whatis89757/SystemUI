...
public class VolumeDialogControllerImpl implements VolumeDialogController, Dumpable {
    ...
    private static final int VIBRATE_HINT_DURATION = 150;
    ...
    // Set media volume below config_safe_media_volume_index on insertion of Headset
    private boolean setSafeVolumeOnHeadset(Intent intent) {
        boolean isPluggedIn = intent.getIntExtra("state", -1) == 1;
        if (isPluggedIn) {
            int currentVolume = mAudio.getStreamVolume(AudioManager.STREAM_MUSIC);
            int mSafeMediaVolumeIndex = mContext.getResources().getInteger(
                    com.android.internal.R.integer.config_safe_media_volume_index);
            if (currentVolume >= mSafeMediaVolumeIndex) {
                mAudio.setStreamVolume(AudioManager.STREAM_MUSIC, mSafeMediaVolumeIndex-1, 0);
            }
        }
        int level = mAudio.getStreamVolume(AudioManager.STREAM_MUSIC);
        return updateStreamLevelW(AudioManager.STREAM_MUSIC, level);
    }
    ...
    private final class Receiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(AudioManager.VOLUME_CHANGED_ACTION)) {
                ...
                changed = setSafeVolumeOnHeadset(intent);
            }
            if (changed) {
                mCallbacks.onStateChanged(mState);
            }
