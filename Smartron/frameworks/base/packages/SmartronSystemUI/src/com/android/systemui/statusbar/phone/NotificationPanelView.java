...
public class NotificationPanelView extends PanelView implements
    ...
    private ImageView mHideKeyguardNotification;
    ...
    protected void onFinishInflate() {
        ...
        mHideKeyguardNotification = findViewById (R.id.show_keyguard_notifications);
        mHideKeyguardNotification.setOnClickListener (this);
    ...
    public void setQsExpansionEnabled(boolean qsExpansionEnabled) {
        mQsExpansionEnabled = qsExpansionEnabled && canExpandQs();
        if (mQs == null) return;
    ...
    public void onClick(View v) {
        ...
        if(v.getId() == R.id.show_keyguard_notifications ){
            boolean isShown = hasShownNotifications();
            Settings.Secure.putInt(mContext.getContentResolver(), Settings.Secure.LOCK_SCREEN_SHOW_HIDE_NOTIFICATIONS_ICON, isShown? 2 : 0);
            updateKeyguardNotificationIcon(isShown);
        }
    }

    public void updateKeyguardNotificationIcon(boolean value){
        mHideKeyguardNotification.setImageResource (value ? R.drawable.ic_hide_keyguard_notification_on : R.drawable.ic_hide_keyguard_notification_off);
     }

    public void updateIconVisibilty(boolean value){
        if(value){
            mHideKeyguardNotification.setVisibility(VISIBLE);
            updateKeyguardNotificationIcon(hasShownNotifications());
        }
        else{
            mHideKeyguardNotification.setVisibility(INVISIBLE);
            mHideKeyguardNotification.setImageResource (0);
        }
    }
    public boolean hasShownNotifications(){
        return Settings.Secure.getInt(mContext.getContentResolver(), Settings.Secure.LOCK_SCREEN_SHOW_HIDE_NOTIFICATIONS_ICON, 0) != 2 ;
    }
        ...
        if (start) {
            ...
            if (mFalsingManager.shouldEnforceBouncer()) {
                mStatusBar.executeRunnableDismissingKeyguard(new Runnable() {
                    ...
                    public void run() {
                        //FACE_UNLOCK_SUPPORT start
                        if (FaceUnlockUtil.getInstance().launchCamera(mLastCameraLaunchSource)) {
                        } else
                        //FACE_UNLOCK_SUPPORT end
    ...
    protected boolean onMiddleClicked() {
        //FACE_UNLOCK_SUPPORT start
        if(FaceUnlockUtil.getInstance().startFaceUnlockByRetry(FaceUnlockUtil.FACE_UNLOCK_RETRY_IN_KEYGUARD)) {
            return true;
        }
        //FACE_UNLOCK_SUPPORT end
    ...
    private boolean canExpandQs() {
        final boolean keyguardOrShadeShowing = mStatusBarState == StatusBarState.KEYGUARD
                || mStatusBarState == StatusBarState.SHADE_LOCKED;
        if (keyguardOrShadeShowing) {
            return mLockScreenQsEnabled || (!mLockScreenQsEnabled && !mLockPatternUtils.isSecure(UserHandle.myUserId()));
        }
        return true;
    }

    public void updateLockScreenQsEnabled(boolean enable) {
       if (enable != mLockScreenQsEnabled) {
           mLockScreenQsEnabled = enable;
           mQsExpansionEnabled = mQsExpansionEnabled && canExpandQs();
        }
    }
}
