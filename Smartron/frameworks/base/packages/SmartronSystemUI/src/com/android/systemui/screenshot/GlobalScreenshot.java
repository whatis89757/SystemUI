...
class SaveImageInBackgroundTask extends AsyncTask<Void, Void, Void> {
    ...
    private static final String NOTIFICATION_SCREENSHOT = "Screenshot_Notification";
    ...
    SaveImageInBackgroundTask(Context context, SaveImageInBackgroundData data,
            NotificationManager nManager,GlobalScreenshot globalScreenshot) {
        Resources r = context.getResources();

        // Prepare all the output metadata
        mParams = data;
        mGlobalScreenshot = globalScreenshot;
    ...
    protected void onPostExecute(Void params) {
        if (mParams.errorMsgResId != 0) {
            ...
            Context context = mParams.context;
            Resources r = context.getResources();
            if(mGlobalScreenshot.mScreenshotView != null)
               mGlobalScreenshot.mScreenshotView.setImageURI(mParams.imageUri);
            ...
            launchIntent.putExtra(NOTIFICATION_SCREENSHOT, true);
        ...
        finisherRun(mParams.finisher);
        mParams.clearContext();
    }

    private void finisherRun(Runnable finisher){
        if(finisher instanceof TakeScreenshotService.FinisherRunnable){
            ((TakeScreenshotService.FinisherRunnable)finisher).clear(true);
            finisher.run();
        }
    }
...
class GlobalScreenshot implements View.OnClickListener{
    ...
    //screenshot visibility for 3 seconds to provide user to perform actions
    private static final int SCREENSHOT_DROP_OUT_DELAY = 3000;
    private static final int SCREENSHOT_DROP_OUT_DURATION = 430;
    private static final int SCREENSHOT_DROP_OUT_SCALE_DURATION = 370;
    private static final int SCREENSHOT_FAST_DROP_OUT_DURATION = 320;
    private static final float BACKGROUND_ALPHA = 0.85f;
    ...
    private ImageButton mShare,mEdit,mDelete;
    LinearLayout mScreenshotActions;
    Uri mUri;
    ...
    public GlobalScreenshot(Context context) {
        ...
        mShare = (ImageButton) mScreenshotLayout.findViewById(R.id.share);
        mEdit = (ImageButton) mScreenshotLayout.findViewById(R.id.Edit);
        mDelete = (ImageButton) mScreenshotLayout.findViewById(R.id.delete);
        mScreenshotActions = (LinearLayout) mScreenshotLayout.findViewById(R.id.actions);
        if (!mContext.getResources().getBoolean(R.bool.show_screenshot_actions_view)) {
                if (mScreenshotActions != null) {
                    mScreenshotActions.setVisibility(View.GONE);
                }
            }
    ...
    private void saveScreenshotInWorkerThread(Runnable finisher) {
        ...
        mSaveInBgTask = (SaveImageInBackgroundTask) new SaveImageInBackgroundTask(mContext, data, mNotificationManager,this)
                .execute();
    }
    ...
    public boolean isAnimationEnd() {
        return mScreenshotAnimation == null || mScreenshotAnimation != null && !(mScreenshotAnimation.isRunning() || mScreenshotAnimation.isStarted());
    }
    ...
    private ValueAnimator createScreenshotDropInAnimation() {
        ...
        anim.addUpdateListener(new AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                mBackgroundView.setAlpha(BACKGROUND_ALPHA);
    ...
    @Override
        public void onClick (View view){
            ...
            switch (view.getId()) {
                case R.id.global_screenshot_background:
                    closeDialogs();
                    mUri = null;
                    return;
                case R.id.share:
                    if (mUri != null) {
                        ...
                        screenshotIntent.putExtra(Intent.EXTRA_STREAM, mUri);
                        mNotificationManager.cancel(SystemMessage.NOTE_GLOBAL_SCREENSHOT);
                        break;
                    } else
                        return;
                case R.id.Edit:
                    if (mUri != null) {
                        screenshotIntent.setAction(Intent.ACTION_EDIT);
                        screenshotIntent.setData(mUri);
                        mNotificationManager.cancel(SystemMessage.NOTE_GLOBAL_SCREENSHOT);
                        break;
                    } else
                        return;
                case R.id.delete:
                    if (mUri != null) {
                        new DeleteImageInBackgroundTask(mContext).execute(mUri);
                        closeDialogs();
                        mNotificationManager.cancel(SystemMessage.NOTE_GLOBAL_SCREENSHOT);
                        Toast.makeText(mContext, R.string.screenshot_deleted_toast_message, Toast.LENGTH_SHORT).show();
                        mUri = null;
                    }
                    return;
            }
            closeDialogs();
            mContext.startActivity(Intent.createChooser(screenshotIntent, null));
            mUri = null;
        }
