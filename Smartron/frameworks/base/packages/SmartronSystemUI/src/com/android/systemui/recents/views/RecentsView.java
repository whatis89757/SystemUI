...
public class RecentsView extends FrameLayout {
    ...
    private View mStackActionButton;
    private TextView mEmptyView;
    private float mStackButtonShadowRadius;
    private PointF mStackButtonShadowDistance;
    private int mStackButtonShadowColor;
    ...
    public RecentsView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        if (RecentsDebugFlags.Static.EnableStackActionButton) {
            ...
            if (Recents.getConfiguration().isLowRamDevice) {
                mStackActionButton = inflater.inflate(R.layout.recents_low_ram_stack_action_button,
                        this, false);
                TextView stackActionView = (TextView) mStackActionButton;
                mStackButtonShadowRadius = stackActionView.getShadowRadius();
                mStackButtonShadowDistance = new PointF(stackActionView.getShadowDx(),
                        stackActionView.getShadowDy());
                mStackButtonShadowColor = stackActionView.getShadowColor();
            } else {
                mStackActionButton = inflater.inflate(R.layout.recents_stack_action_button,
                        this, false);
            }
    ...
    public void reevaluateStyles() {
        ...
        if (Recents.getConfiguration().isLowRamDevice && mStackActionButton != null) {
            TextView stackActionView = (TextView) mStackActionButton;
            stackActionView.setTextColor(textColor);
            // Enable/disable shadow if text color is already dark.
            if (usingDarkText) {
                stackActionView.setShadowLayer(0, 0, 0, 0);
            } else {
                stackActionView.setShadowLayer(mStackButtonShadowRadius,
    ...
    public void updateStack(TaskStack stack, boolean setStackViewTasks) {
        ...
        if (stack.getTaskCount() > 0) {
            hideEmptyView();
            showStackActionButton(SHOW_STACK_ACTION_BUTTON_DURATION, true);
        } else {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (RecentsDebugFlags.Static.EnableStackActionButton) {
            ...
            if (Recents.getConfiguration().isLowRamDevice) {
                ...
                int navBarHeight = mIsInMultiWindow ? 0 : mContext.getResources().
                        getDimensionPixelSize(com.android.internal.R.dimen.navigation_bar_height);
                if (rotation == Surface.ROTATION_90) {
                    right = right - navBarHeight;
                } else if (rotation == Surface.ROTATION_270) {
                    left =  left + (navBarHeight / 2);
                } else {
                    bottom = bottom - navBarHeight;
                }

                int buttonWidth = mStackActionButton.getMeasuredWidth() - ((int)Math.round(Math.sqrt(mStackActionButton.getMeasuredWidth())))/* padding */;
                mRect = new Rect(right - ((right - buttonWidth) / 2) - buttonWidth + left, bottom - buttonWidth, right -((right - buttonWidth) / 2) + left, bottom);
                mStackActionButton.layout(mRect.left, mRect.top, mRect.right, mRect.bottom);
            }
        }
    ...
    public final void onBusEvent(MultiWindowStateChangedEvent event) {
        mIsInMultiWindow = event.inMultiWindow;
