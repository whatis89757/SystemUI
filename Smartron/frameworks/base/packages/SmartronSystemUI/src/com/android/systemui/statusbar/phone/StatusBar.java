...
public class StatusBar extends SystemUI implements DemoMode,
    ...
    private boolean mLockScreenQsEnabled = true;
    ...
    private final ContentObserver mLockScreenQsEnabler = new ContentObserver(new Handler()) {
        @Override
        public void onChange(boolean selfChange) {
            updateLockScreenQsEnabled();
        }
    };

    private void updateLockScreenQsEnabled() {
        mLockScreenQsEnabled  = Settings.Secure.getInt(mContext.getContentResolver(),
                Settings.Secure.LOCK_SCREEN_QS_PANEL_ENABLED, 0) != 1;
        if (mNotificationPanel != null) {
            mNotificationPanel.updateLockScreenQsEnabled(mLockScreenQsEnabled);
        }
    }

    private boolean isLockScreenQsEnabled() {
        return mLockScreenQsEnabled || (!mLockScreenQsEnabled && !mLockPatternUtils.isSecure(UserHandle.myUserId()));
    }
    ...
    public void start() {
        ...
        mContext.getContentResolver().registerContentObserver(
                ...
                UserHandle.USER_ALL);
        mContext.getContentResolver().registerContentObserver(
                Settings.Secure.getUriFor(Settings.Secure.LOCK_SCREEN_SHOW_HIDE_NOTIFICATIONS_ICON), false,
                mLockscreenSettingsObserver,
                UserHandle.USER_ALL);
        ...
        updateLockScreenQsEnabled();
        mContext.getContentResolver().registerContentObserver(
                    Settings.Secure.getUriFor(Settings.Secure.LOCK_SCREEN_QS_PANEL_ENABLED), true,
                    mLockScreenQsEnabler);
    }
    ...
    protected void startKeyguard() {
        ...
        //FACE_UNLOCK_SUPPORT start
        FaceUnlockUtil.getInstance().setStatusBarKeyguardViewManager(mStatusBarKeyguardViewManager);
        FaceUnlockUtil.getInstance().setKeyguardIndicationController(mKeyguardIndicationController);
        FaceUnlockUtil.getInstance().setKeyguardBottomAreaView(mNotificationPanel.getKeyguardBottomAreaView());
        //FACE_UNLOCK_SUPPORT end
    ...
    protected void setAreThereNotifications() {
        ...
        if (mStatusBarView != null) {
            ...
            final boolean showNotifications = Settings.Secure.getIntForUser(mContext.getContentResolver(),
                Settings.Secure.LOCK_SCREEN_SHOW_HIDE_NOTIFICATIONS_ICON,
                0,
                mCurrentUserId) != 1 ;
            mNotificationPanel.updateIconVisibilty(hasActiveNotifications() && showNotifications);
    ...
    //isAll : to bypass all the checks or listeners of back so that to close all panels
    public boolean onBackPressed(boolean isAll) {
        if (mStatusBarKeyguardViewManager.onBackPressed() && !isAll) {
            return true;
        }
    ...
    public boolean onDraggedDown(View startingChild, int dragLengthY) {
        if (mState == StatusBarState.KEYGUARD
                && hasActiveNotifications() && (!isDozing() || isPulsing()) && isLockScreenQsEnabled()) {
            mLockscreenGestureLogger.write(
    ...
    public void onCameraLaunchGestureDetected(int source) {
        ...
        if (isGoingToSleep()) {
            ...
            //FACE_UNLOCK_SUPPORT start
            FaceUnlockUtil.getInstance().resetPowerKey();
            //FACE_UNLOCK_SUPPORT start
    ...
    private void updateLockscreenNotificationSetting() {
        final int showHideIcon = Settings.Secure.getIntForUser(mContext.getContentResolver(),
                Settings.Secure.LOCK_SCREEN_SHOW_HIDE_NOTIFICATIONS_ICON,
                0,
                mCurrentUserId);
        mNotificationPanel.updateIconVisibilty(showHideIcon != 1);
        ...
        setShowLockscreenNotifications((showHideIcon == 0) && allowedByDpm);
