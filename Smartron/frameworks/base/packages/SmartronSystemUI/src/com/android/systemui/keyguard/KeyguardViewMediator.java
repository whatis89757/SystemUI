...
public class KeyguardViewMediator extends SystemUI {
    ...
    private void playSounds(boolean locked) {
        //FACE_UNLOCK_SUPPORT start
        if (FaceUnlockUtil.getInstance().isKeyguardUnlocked(locked)) {
    ...
    private void playSound(int soundId) {
        ...
        if (Settings.System.getInt(cr, Settings.System.LOCKSCREEN_SOUNDS_ENABLED, 1) == 1) {
            ...
            //Log.d(TAG, "playSound, soundId = "+soundId+", mLockSoundStreamId = "+mLockSoundStreamId);
    ...
    private boolean isFaceUnlockOn(){
        return Settings.Secure.getInt(mContext.getContentResolver(),Settings.Secure.FACE_UNLOCK_SUPPORT,0) == 1;
    }

    private void startFaceUnlockService(boolean isBoot){
        if(mContext == null || !isFaceUnlockOn()){
            return;
        }
        final Intent intent = new Intent("com.smartron.faceunlockservice");
        intent.putExtra("isBoot",isBoot);
        intent.setPackage("com.smartron.faceunlock");
    ...
    public void onBootCompleted() {
        ...
        synchronized (this) {
            ...
            if(mSystemReady && mLockPatternUtils.isSecure(
                    KeyguardUpdateMonitor.getCurrentUser())) {
                Log.d(TAG,"User locked and starting SM service");
                startFaceUnlockService(true);
            }
        }
    }
