...
public class UserSwitcherController {
    ...
    private static final String WIFI_AP_USER_SET = "wifi_ap_user_config_set";
        ...
        new AsyncTask<SparseArray<Bitmap>, Void, ArrayList<UserRecord>>() {
            ...
            protected ArrayList<UserRecord> doInBackground(SparseArray<Bitmap>... params) {
                ...
                for (UserInfo info : infos) {
                    ...
                    if(info.id ==UserHandle.USER_OWNER){
                        updateWifiConfigAPStore(info.name);
                    }
    ...
    private void updateWifiConfigAPStore(String userName){
        boolean isConfigureSet = Settings.Secure.getInt(mContext.getContentResolver(),WIFI_AP_USER_SET,-1)!=-1;
        if(!isConfigureSet){
            WifiManager wifiManager = (WifiManager) mContext.getSystemService(Context.WIFI_SERVICE);
            WifiConfiguration config = wifiManager.getWifiApConfiguration();
            if(config!=null){
                config.SSID =
                        mContext.getResources().getString(com.android.internal.R.string.sm_wifi_ap_config_name,userName,
                        Build.MODEL);
                Log.d(TAG,"AP config setting "+config.SSID);
