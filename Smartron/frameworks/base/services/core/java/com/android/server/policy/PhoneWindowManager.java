...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    private long mCurrentFpEventTime = 0;
    private static final int FP_DEVICE_ID = 0;
    private static final int FP_LONG_PRESS_LOCK = 2;
    private static final int FP_DOUBLE_TAP_LOCK = 1;
    private static final int FP_DISABLE_LOCK = 0;
    private static final int INVALID_DEVICE_ID = -2;
    private int mTPCurrentDeviceID = INVALID_DEVICE_ID;
    ...
    private boolean mScreenshotGestureEnabled = false;
    ...
    private static final int MSG_LAUNCH_CAMERA = 27;
    ...
    class SettingsObserver extends ContentObserver {
        ...
        void observe() {
            ...
            resolver.registerContentObserver(Settings.Global.getUriFor(
                    ...
                    UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.Global.getUriFor(
                    Settings.Global.THREE_FINGER_SCREENSHOT_GESTURE_ENABLED), false, this,
                    UserHandle.USER_ALL);
            updateSettings();
        }
    ...
    private FingerSwipeGestureListener mThreeFingerSwipeListener;
    ...
    private void interceptPowerKeyDown(KeyEvent event, boolean interactive) {
        ...
        if (telecomManager != null && event.getDeviceId() != FP_DEVICE_ID) {
            if (telecomManager.isRinging()) {
    ...
    private long getScreenshotChordLongPressDelay() {
        if (mKeyguardDelegate.isShowing()) {
            ...
            return (long) (KEYGUARD_SCREENSHOT_CHORD_DELAY_MULTIPLIER *
                    ViewConfiguration.get(mContext).getScreenshotTimeout());
        }
        return ViewConfiguration.get(mContext).getScreenshotTimeout();
    }
    ...
    private void launchCamera(){
        ...
        if (statusbar != null) {
            statusbar.onCameraLaunchGestureDetected(StatusBarManager.CAMERA_LAUNCH_SOURCE_WIGGLE);
        }
    }
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        mThreeFingerSwipeListener = new FingerSwipeGestureListener(context, new FingerSwipeGestureListener.Callbacks() {
            @Override
            public void onThreeFingerSwipeDetected() {
        ...
        mSystemGestures = new SystemGesturesPointerEventListener(context,
                new SystemGesturesPointerEventListener.Callbacks() {
                    ...
                    public void onSwipeFromBottom() {
                        //add by jiangfeng for cit.
                        if (isRunning(mContext, HCIT_PACKAGE_NAME) && isGestureIsolated()) {
                            Log.d(TAG,"CIT package running, onSwipeFromBottom() ignored..");
                            return;
                        }
                    ...
                    public void onSwipeFromRight() {
                        if (isGestureIsolated() && isRunning(mContext, HCIT_PACKAGE_NAME)) {
                            Log.d(TAG,"CIT package running, onSwipeFromRight() ignored..");
                            return;
                        }
    ...
    private int deviceID(){
        ...
        int[] inputManagerDeviceList = inputManager.getInputDeviceIds ();
        for(int deviceId : inputManagerDeviceList){
    ...
    private void enableSwipeScreenshotGesture(boolean enable){
        if (enable) {
            if (mScreenshotGestureEnabled) return;
            mScreenshotGestureEnabled = true;
            mWindowManagerFuncs.registerPointerEventListener(mThreeFingerSwipeListener);
            Log.d(TAG,"Registering Three finger swipe gesture to WindowMangerFuncs...");
        } else {
            if (!mScreenshotGestureEnabled) return;
            mScreenshotGestureEnabled = false;
            mWindowManagerFuncs.unregisterPointerEventListener(mThreeFingerSwipeListener);
            Log.d(TAG,"Unregistering Three finger swipe gesture from WindowMangerFuncs...");
        }
    }
    ...
    public void updateSettings() {
        ...
        synchronized (mLock) {
            ...
            enableSwipeScreenshotGesture(isThreeFingerSwipeScreenshotEnabled());
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        if (keyCode == KeyEvent.KEYCODE_HOME) {
            ...
            if (down && lockDeviceWithFpEnabled(FP_DOUBLE_TAP_LOCK)) {
                if (keyguardOn && mFingerprintManager != null && mFingerprintManager.hasEnrolledFingerprints()) {
                    return -1;
                 } else {
                     //Lock device with two successive FP events.
                     lockDeviceWithFp(event.getDownTime());
                 }
             }
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
        ...
        if (isValidGlobalKey(keyCode)
            ...
            if (isWakeKey) {
                Log.d("SMWakeup","interceptKeyBeforeQueueing "+keyCode);
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_CAMERA: {
                //Log.d(TAG,"Fingerprint Event : keyDown = "+down +"  interactive = "+interactive +" fingerprint Wake option Enabled = "+fingerprintWakeEnabled());
                if(!interactive && mTPCurrentDeviceID == INVALID_DEVICE_ID){
                    mTPCurrentDeviceID = deviceID();
                }
                if (down && !interactive && event.getDeviceId() == mTPCurrentDeviceID) {
                    int value = Settings.Global.getInt(mContext.getContentResolver(), Settings.Global.DRAW_SCREEN_OVERLAY_C, 0);
                    if (1 == value) isWakeKey = true;
                    openCameraUsingDrawScreenOverlay(value);
                }
                break;
            }
            ...
            case KeyEvent.KEYCODE_POWER: {
                if (mTPCurrentDeviceID == INVALID_DEVICE_ID) {
                    mTPCurrentDeviceID = deviceID();
                }
                if (event.getDeviceId() == mTPCurrentDeviceID) {
                    final boolean up = event.getAction() == KeyEvent.ACTION_UP;
                    if (up) break;
                    TelecomManager telecomManager = getTelecommService();
                    if (telecomManager != null && telecomManager.isInCall()) {
                        Log.d(TAG,"Drop double tap screen event, due to telecom call in progress");
                        break;
                    }
                    boolean value = Settings.Global.getInt(mContext.getContentResolver(),
                                            Settings.Global.DOUBLE_TAP_SCREEN_TO_WAKE_ENABLED, 0) == 1;
                    if (!value) break;
                }

                if (isRunning(mContext, HCIT_PACKAGE_NAME)
                        && SystemProperties.getInt("sys.hymost.cit.keytest", 0) == 1) {
                    Log.v(TAG,"intercept power key");
                    break;
                }
        ...
        if (isWakeKey) {
            Log.d("SMWakeup","interceptKeyBeforeQueueing After process "+keyCode);
            wakeUp(event.getEventTime(), mAllowTheaterModeWakeFromKey, "android.policy:KEY");
        }
    ...
    private void openCameraUsingDrawScreenOverlay(int value){
        if(0 == value) return;
        Log.i(TAG,"Launching camera with touch gesture");
        mHandler.removeMessages(MSG_LAUNCH_CAMERA);
        Message msg = mHandler.obtainMessage(MSG_LAUNCH_CAMERA);
        msg.setAsynchronous(true);
        msg.sendToTarget();
    }

    private boolean isThreeFingerSwipeScreenshotEnabled() {
         return Settings.Global.getInt(mContext.getContentResolver(),
                Settings.Global.THREE_FINGER_SCREENSHOT_GESTURE_ENABLED, 0) == 1;
     }

     //Check whether lock with fingerprint enabled.
     private boolean lockDeviceWithFpEnabled(int lockId) {
         return Settings.Global.getInt(mContext.getContentResolver(),
                Settings.Global.FINGERPRINT_LOCK_ENABLED, FP_DISABLE_LOCK) == lockId;
     }

    // lock device with two successive events of FP.
    private void lockDeviceWithFp(long eventTime) {
        long previousFpEventTime = mCurrentFpEventTime;
        mCurrentFpEventTime = eventTime;
        long deltaTime = mCurrentFpEventTime - previousFpEventTime;
        //Differece between two successive FP events would be between 150ms and 400ms
        if (deltaTime > 150 && deltaTime < 400) {
            if (!mTopIsFullscreen) {
    ...
    //Check whether wake with fingerprint enabled.
     private boolean fingerprintWakeEnabled() {
         return Settings.Global.getInt(mContext.getContentResolver(),
    ...
    //add by jiangfeng for cit begin
    private static final String HCIT_PACKAGE_NAME = "hymost.android.cit";

    public static boolean isRunning(Context context, String packageName) {
        ActivityManager am = (ActivityManager) context.getSystemService(Context.ACTIVITY_SERVICE);
        List<ActivityManager.RunningTaskInfo> list = am.getRunningTasks(100);
        for (ActivityManager.RunningTaskInfo info : list) {
            Log.d(TAG, "info.topActivity.getPackageName()--->" + info.topActivity.getPackageName());
            if (info.topActivity.getPackageName().equals(packageName) && info.baseActivity.getPackageName().equals(packageName)) {
                //find it, break
                Log.d(TAG, "info.topActivity.getPackageName()--->true");
                return true;
            }
        }
        return false;
    ...
    private boolean wakeUp(long wakeTime, boolean wakeInTheaterMode, String reason) {
        //FACE_UNLOCK_SUPPORT start
        Slog.i("FaceUnlockUtil", "PhoneWindowManager wakeUp");
        Settings.System.putInt(mContext.getContentResolver(), "faceunlock_start", 1);
        //FACE_UNLOCK_SUPPORT end
