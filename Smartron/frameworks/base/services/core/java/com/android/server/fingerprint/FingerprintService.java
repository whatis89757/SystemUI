...
//add by sunwave begin
	private IFingerprintServiceReceiver mMmiReceiver;
//add by sunwave end
...
//add by sunwave begin
	protected void handleSendCmd(int cmd, int arg, int len, ArrayList<Byte> data) {
		Slog.d(TAG, "handleSendCmd: cmd = " + cmd +"(0x"+Integer.toHexString(cmd)+")");
		
		final byte[] buf = new byte[data.size()];
        for (int i = 0; i < data.size(); i++) {
            buf[i] = data.get(i);
        }
		if (mMmiReceiver == null){
			Slog.w(TAG, "handleSendCmd: mMmiReceiver is null");
		} else {
			try {
				mMmiReceiver.onSendCmd(cmd, arg, len, buf);
			} catch (RemoteException e) {
				Slog.w(TAG, "Failed to invoke onSendCmd:", e);
			}
		}	
	}
//add by sunwave end
...
//add by sunwave begin
        ...
        public void onSendCmd(final int cmd, final int arg, final int len, ArrayList<Byte> data) {
			Slog.d(TAG, "onSendCmd: cmd = " + cmd +"(0x"+Integer.toHexString(cmd)+")");
            mHandler.post(new Runnable() {
                @Override
                public void run() {
                    handleSendCmd(cmd, arg, len, data);
                }
            });
        }
//add by sunwave end
...
//add by sunwave begin
        ...
        public int sendCmd(int cmd, int param, int len) {
            Slog.d(TAG, "sendCmd: cmd = " + cmd +"(0x"+Integer.toHexString(cmd)+")" + " param = "+ param + " len = "+ len);
			IBiometricsFingerprint daemon = getFingerprintDaemon();
            if (daemon == null) {
                Slog.d(TAG, "sendCmd: daemon is null!");
                return -1;
            }
            try {
                return daemon.sendCmd(cmd,param,len);
            } catch (RemoteException e) {
                Slog.e(TAG, "Remote exception in sendCmd: ", e);
            }
            return -1;
        }
		
		@Override // Binder call
        public void onSendCmd(final IFingerprintServiceReceiver receiver) {
			mMmiReceiver = receiver;
        }
//add by sunwave end
