...
public final class PowerManagerService extends SystemService
    ...
    private final class BinderService extends IPowerManager.Stub {
        ...
        /**
         *Turn turnFlashStatus on/off
         * @hide
         */
        @Override // Binder call
        public void turnFlashStatus(int id,int status) {
            final String COMMON_FLASHLIGHT_BRIGHTNESS_0 = "/sys/class/leds/led:torch_0/brightness";
        	final String COMMON_FLASHLIGHT_BRIGHTNESS_1 = "/sys/class/leds/led:torch_1/brightness";
        	final String FLASHLIGHT_SWITCH = "/sys/class/leds/led:switch/brightness";
        	final String FRONT_FLASHLIGHT1 = "/sys/class/pwm/pwmchip0/device/config";
        	final String FRONT_FLASHLIGHT2 = "/sys/class/pwm/pwmchip0/device/brightness";
        	final int FLASH_ID_MAIN = 0;
        	final int FLASH_ID_SLAVE = 1;
        	final int FLASH_ID_FRONT = 2;

        	final byte[] LIGHTE_ON = { '1'};
        	final byte[] LIGHTE_OFF = { '0' };
        	final byte[] F1_LIGHTE_ON = { '5','0'};
        	final byte[] F1_LIGHTE_OFF = { '0' };

        	final int FLASH_ID = id;
        	final int FLASH_STATU= status;
			switch (FLASH_ID) {
			case 0:
				if (0 == FLASH_STATU) {
					writeFile(COMMON_FLASHLIGHT_BRIGHTNESS_0, LIGHTE_OFF);
					writeFile(FLASHLIGHT_SWITCH, LIGHTE_OFF);
				} else if (1 == FLASH_STATU) {
					writeFile(COMMON_FLASHLIGHT_BRIGHTNESS_0, LIGHTE_ON);
					writeFile(FLASHLIGHT_SWITCH, LIGHTE_ON);
				}
				break;
			case 1:
				if (0 == FLASH_STATU) {
					writeFile(COMMON_FLASHLIGHT_BRIGHTNESS_1, LIGHTE_OFF);
					writeFile(FLASHLIGHT_SWITCH, LIGHTE_OFF);
				} else if (1 == FLASH_STATU) {
					writeFile(COMMON_FLASHLIGHT_BRIGHTNESS_1, LIGHTE_ON);
					writeFile(FLASHLIGHT_SWITCH, LIGHTE_ON);
				}
				break;
			case 2:
				if (0 == FLASH_STATU) {
					writeFile(FRONT_FLASHLIGHT1, F1_LIGHTE_OFF);
					writeFile(FRONT_FLASHLIGHT2, LIGHTE_OFF);
				} else if (1 == FLASH_STATU) {
					writeFile(FRONT_FLASHLIGHT1, F1_LIGHTE_ON);
					writeFile(FRONT_FLASHLIGHT2, LIGHTE_ON);
				}
				break;
        ...
        /**
         *Turn turnUpIndicator on/off
         * @hide
         */
        @Override // Binder call
        public void turnUpIndicator(int color) {
        	
        	final String GREEN_FILE_PATH = "sys/class/leds/green/brightness";
        	final String RED_FILE_PATH = "sys/class/leds/red/brightness";
        	final String BLUE_FILE_PATH = "sys/class/leds/blue/brightness";
            final int COLOR_GREEN = 0;
            final int COLOR_RED = 1;
            final int COLOR_BLUE = 2;
            final int COLOR_ORANGE = 3;
            final int COLOR_CLOSE = 4;

        	final byte[] ON = { '2','5','5' };
        	final byte[] OFF = { '0' };
            switch (color) {
    		case COLOR_GREEN:
    			writeFile(BLUE_FILE_PATH, OFF);
                writeFile(GREEN_FILE_PATH, ON);
                writeFile(RED_FILE_PATH, OFF);
    			break;
    		case COLOR_RED:
    			writeFile(BLUE_FILE_PATH, OFF);
                writeFile(GREEN_FILE_PATH, OFF);
                writeFile(RED_FILE_PATH, ON);
    			break;
    		case COLOR_BLUE:
    			writeFile(BLUE_FILE_PATH, ON);
                writeFile(GREEN_FILE_PATH, OFF);
                writeFile(RED_FILE_PATH, OFF);
    			break;
    		case COLOR_ORANGE:
         ...
         * @hide
         */
        private void writeFile (String path,byte[] date) {
            FileOutputStream red = null;
            try {
                red = new FileOutputStream(path);
                red.write(date);
                red.flush();
            } catch (IOException e) {
                e.printStackTrace();
