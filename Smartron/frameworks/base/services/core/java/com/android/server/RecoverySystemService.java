...
public final class RecoverySystemService extends SystemService {
    ...
    private final class BinderService extends IRecoverySystem.Stub {
        ...
        // rock debug
        public boolean writeToFile(String command,File filePath) {
            deleteFile(filePath);
            try (FileWriter commandFile = new FileWriter(filePath)) {
                commandFile.write(command + "\n");
                if (DEBUG) {
                    Slog.d(TAG, " writeToFile : command : \n" + command );
                    Slog.d(TAG, " writeToFile : write" + filePath.getAbsolutePath() + " done");
                }
            } catch (IOException e) {
                Slog.e(TAG, "IOException when writing \"" +
        ...
        public boolean deleteFile(File filePath) {
            try {
                File commandFile = filePath;
                if(commandFile.exists()) {
                      commandFile.delete();
                      if (DEBUG) Slog.i(TAG, " deleteFile : " + filePath.getAbsolutePath() + " delete done");
                } else {
                    if (DEBUG) Slog.i(TAG, " deleteFile : " + filePath.getAbsolutePath() + " not found");
                    return false;
                }
            } catch (Exception e) {
                Slog.e(TAG, "Exception when deleting \"" +
                        filePath.getAbsolutePath() + "\":", e);
                return false;
            }
            return true;
        ...
        private boolean setupOrClearBcb(boolean isSetup, String command) {
            ...
            try {
                ...
                if (isSetup) {
                    ...
                    dos.flush();
                    if (!writeToFile(command,RecoverySystem.COMMAND_FILE))
                            Slog.e(TAG, " setupOrClearBcb : Failed to write COMMAND_FILE");
                ...
                if (status == 100) {
                    ...
                    // Error in /system/bin/uncrypt.
                    Slog.e(TAG, "uncrypt failed with status: " + status);
                    if(!deleteFile(RecoverySystem.COMMAND_FILE))
                            Slog.e(TAG, " setupOrClearBcb : Failed to delete COMMAND_FILE");
                    return false;
                }
            } catch (IOException e) {
