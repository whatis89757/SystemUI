...
public class StatusBarManagerService extends IStatusBarService.Stub {
    ...
    public void shutdown() {
        shutdownWithConfirm(false);
    }

    /**
     * Allows the status bar to shutdown the device with confirmation.
     */
    @Override
    public void shutdownWithConfirm(boolean confirm) {
        enforceStatusBarService();
        long identity = Binder.clearCallingIdentity();
        try {
    ...
    public void reboot(boolean safeMode) {
        if (safeMode) {
            rebootSafeMode(false);
        } else {
            rebootWithConfirm(false);
        }
    }

    /**
     * Allows the status bar to reboot the device with confirmation.
     */
    @Override
    public void rebootWithConfirm(boolean confirm) {
        enforceStatusBarService();
        long identity = Binder.clearCallingIdentity();
        try {
            mHandler.post(() -> {
    ...
    /**
     * Allows the status bar to reboot the device to safeMode with confirmation.
     */
    @Override
