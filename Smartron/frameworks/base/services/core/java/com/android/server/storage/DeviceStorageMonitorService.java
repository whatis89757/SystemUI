...
public class DeviceStorageMonitorService extends SystemService {
    ...
    private AlertDialog mLowStorageDialog = null;
    /**
    ...
    private void check() {
        ...
        for (VolumeInfo vol : storage.getWritablePrivateVolumes()) {
            ...
            if (StorageManager.UUID_DEFAULT.equals(uuid) && (usableBytes <= lowBytes || mForceLevel != State.LEVEL_UNKNOWN)) {
                Slog.w(TAG, "fullBytes = " + fullBytes);
                Slog.w(TAG, "lowBytes = " + lowBytes);
                Slog.w(TAG, "usableBytes = " + usableBytes);
                Slog.w(TAG, "mForceLevel = " + State.levelToString(mForceLevel));
                Slog.w(TAG, "uuid = " + uuid);
                Slog.w(TAG, "UUID_DEFAULT = " + StorageManager.UUID_DEFAULT);
                Slog.w(TAG, "isBootImageOnDisk = " + isBootImageOnDisk());
                alertLowStorage();
            }
    ...
    private void alertLowStorage() {
        final Context context = getContext();
        if ((mLowStorageDialog != null && mLowStorageDialog.isShowing())) {
            return;
        }
        AlertDialog.Builder builder = new AlertDialog
            .Builder(context);
        mLowStorageDialog = builder.setTitle(com.android.internal.R.string.low_internal_storage_view_title)
                .setMessage(com.android.internal.R.string.low_internal_storage_view_text)
        ...
        mLowStorageDialog.getWindow().setType(
                WindowManager.LayoutParams.TYPE_SYSTEM_DIALOG);
        mLowStorageDialog.show();
    }
