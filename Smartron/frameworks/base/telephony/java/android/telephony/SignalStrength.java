...
public class SignalStrength implements Parcelable {
    ...
    /** @hide */
    public boolean isX83_X84 = ( (SystemProperties.get("ro.product.device","0")).equals("rimoE")
            || (SystemProperties.get("ro.product.device","0")).equals("rimoY") )? true : false;

    private static final int[] LTE_THRESH_X83_X84 =   new int[] {-140, -128, -120, -115, -99, -44};//CUSTOMER STANDARD
    private static final int[] GSM_THRESH_X83_X84 =   new int[] {-140, -105, -101, -97,  -89, -44};//HY STANDARD
    private static final int[] WCDMA_THRESH_X83_X84 = new int[] {-140, -102, -100, -94,  -90, -44};//HY STANDARD
    /** @hide */
    ...
    public int getLevel() {
        ...
        if (isGsm) {
	        ...
	        if (DBG) log("cdmaLevel=" + cdmaLevel+" evdoLevel:= "+evdoLevel);	
    ...
    public int getGsmLevel() {
        ...
        int[] threshWcdma; 
        if(isX83_X84){
            threshGsm = GSM_THRESH_X83_X84;
	    threshWcdma = WCDMA_THRESH_X83_X84;	
		if (DBG) log("getGsmLevel---networktype=" + getNetworkType());
		...
		if (networktype == TelephonyManager.NETWORK_TYPE_UMTS ||
		    ...
		    networktype == TelephonyManager.NETWORK_TYPE_HSUPA){
              if (dbm > threshWcdma[5]) level = SIGNAL_STRENGTH_NONE_OR_UNKNOWN;
              else if (dbm >= threshWcdma[4]) level = SIGNAL_STRENGTH_GREAT;
              else if (dbm >= threshWcdma[3]) level = SIGNAL_STRENGTH_GOOD;
              else if (dbm >= threshWcdma[2]) level = SIGNAL_STRENGTH_MODERATE;
              else if (dbm >= threshWcdma[1]) level = SIGNAL_STRENGTH_POOR;
              else if (dbm >= threshWcdma[0]) level = SIGNAL_STRENGTH_NONE_OR_UNKNOWN;
		}else{		
	...
	int networktype = 0;
        ...
        networktype = SystemUtil.getNetworkType();
	return networktype;
    }
    /**
	...
	if (DBG) log("getCdmaLevel---networktype=" + getNetworkType());
        if(isX83_X84){
            if (cdmaDbm >= -90) levelDbm = SIGNAL_STRENGTH_GREAT;
            else if (cdmaDbm >= -94) levelDbm = SIGNAL_STRENGTH_GOOD;
            else if (cdmaDbm >= -100) levelDbm = SIGNAL_STRENGTH_MODERATE;
            else if (cdmaDbm >= -102) levelDbm = SIGNAL_STRENGTH_POOR;
            else levelDbm = SIGNAL_STRENGTH_NONE_OR_UNKNOWN;
        }
        if (DBG) log("getCdmaLevel  levelDbm=" + levelDbm+",isX83_X84="+isX83_X84);
        if (levelDbm != -1) return levelDbm;
        // Ec/Io are in dB*10
        ...
        if (DBG) log("getCdmaLevel=" + level+",levelDbm="+levelDbm+",levelEcio="+levelEcio);
        return level;
    }
