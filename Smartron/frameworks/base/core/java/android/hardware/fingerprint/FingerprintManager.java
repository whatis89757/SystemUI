...
public class FingerprintManager {
   ...
   /**
     ...
     * @hide
     */
    public static final int FINGERPRINT_ERROR_REPEAT_FINGER = 1005;

    /**
     * @hide
     */
    public static final int FINGERPRINT_ERROR_REPEAT_FINGER_CODE = 5;

    /**
     * @hide
     */
    public static final int FINGERPRINT_ERROR_REPEAT_REGION = 1006;

    /**
     * @hide
     */
    public static final int FINGERPRINT_ERROR_REPEAT_REGION_CODE = 6;
...
//add by sunwave begin
	private SendCmdCallback mSendCmdCallback;
//add by sunwave end
...
//add by sunwave begin
	public static abstract class SendCmdCallback {
		/**
		 * sunwave onSendCmdCallback
		 * @hide
		 */
...
//add by sunwave begin
    ...
    public int sendCmd(int cmd, int param, int len) {
        int result = -1;
        Log.d(TAG, "sendCmd: cmd = " + cmd+"(0x"+Integer.toHexString(cmd)+")");
        if (mService != null) try {
			Log.d(TAG, "sendCmd: mService connected!");
            result = mService.sendCmd(cmd, param, len);
        } catch (RemoteException e) {
			Log.e(TAG, "Remote exception in sendCmd: ", e);
            throw e.rethrowFromSystemServer();
        } else {
			Log.d(TAG, "sendCmd: mService not connected!");
		}
        return result;
    }
     ...
     * @hide
     */
	public void onSendCmd(@NonNull SendCmdCallback callback){
		if (mService != null) try {
			Log.d(TAG, "onSendCmd: mService connected!");
			mSendCmdCallback = callback;
            mService.onSendCmd(mServiceReceiver);
        } catch (RemoteException e) {
			Log.e(TAG, "Remote exception in onSendCmd: ", e);
            throw e.rethrowFromSystemServer();
        } else {
			Log.d(TAG, "onSendCmd: mService not connected!");
		}
	}
...
//add by sunwave end
    ...
    private IFingerprintServiceReceiver mServiceReceiver = new IFingerprintServiceReceiver.Stub() {
        ...
        public void onAcquired(long deviceId, int acquireInfo, int vendorCode) {
            if (FINGERPRINT_ERROR_REPEAT_FINGER_CODE == vendorCode){
                mHandler.obtainMessage(MSG_ERROR, FINGERPRINT_ERROR_REPEAT_FINGER, 0, deviceId).sendToTarget();
            } else if (FINGERPRINT_ERROR_REPEAT_REGION_CODE == acquireInfo) {
                mHandler.obtainMessage(MSG_ERROR, FINGERPRINT_ERROR_REPEAT_REGION, 0, deviceId).sendToTarget();
            } else{
...
//add by sunwave begin
		@Override // binder call
        public void onSendCmd(int cmd, int arg, int len, byte[] data) {
			if (mSendCmdCallback != null) {
                mSendCmdCallback.onSendCmdCallback(cmd, arg, len, data);
            }
        }
//add by sunwave end
    };
