...
public class VideoView extends SurfaceView
    ...
    private List<String> mMediaMimeTypes = null;
    ...
    MediaPlayer.OnPreparedListener mPreparedListener = new MediaPlayer.OnPreparedListener() {
        public void onPrepared(MediaPlayer mp) {
            ...
            checkAndSetSupportedAudioTrack();
    ...
    //Get all supported Media mime types and store in a List.
    private void listSupportedMediaMimeTypes() {
        MediaCodecList mediaCodecList = new MediaCodecList(MediaCodecList.ALL_CODECS);
        MediaCodecInfo [] infos = mediaCodecList.getCodecInfos();
        mMediaMimeTypes = new ArrayList<String>();
        if (infos != null) {
            for (MediaCodecInfo mediaCodecInfo: infos) {
                if (mediaCodecInfo == null) continue;
                for (String types : mediaCodecInfo.getSupportedTypes()) {
                    if (types == null) continue;
                     mMediaMimeTypes.add(types);
                }
            }
        }
    }

    //Check and set supported Audio track to mMediaPlayer.
    private void checkAndSetSupportedAudioTrack() {
        try{
            MediaPlayer.TrackInfo[] trackInfo = mMediaPlayer.getTrackInfo();
            if (trackInfo != null && trackInfo.length > 0 && mMediaMimeTypes != null) {
               for (int i = 0; i < trackInfo.length ; i++ ) {
                    if (TrackInfo.MEDIA_TRACK_TYPE_AUDIO == trackInfo[i].getTrackType() && mMediaMimeTypes.contains(trackInfo[i].getMimeType())) {
