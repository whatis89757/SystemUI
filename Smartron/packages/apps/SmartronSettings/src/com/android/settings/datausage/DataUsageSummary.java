...
public class DataUsageSummary extends DataUsageBase implements Indexable, DataUsageEditController {
    ...
    public void onCreate(Bundle icicle) {
        ...
        if (hasMobileData) {
            ...
            if (subscriptions == null || subscriptions.size() == 0) {
                addMobileSection(defaultSubId, -1, null);
            }
            for (int i = 0; subscriptions != null && i < subscriptions.size(); i++) {
                addMobileSection(subscriptions.get(i).getSubscriptionId(),subscriptions.get(i).getSimSlotIndex(), subscriptions.get(i).getDisplayName());
            }
    ...
    private void addMobileSection(int subId, int slotId, CharSequence displayName) {
        TemplatePreferenceCategory category = (TemplatePreferenceCategory)
                inflatePreferences(R.xml.data_usage_cellular);
        if (slotId == -1 || Utils.isAirplaneModeOn(getContext())) {
            mCellDataPreference = (CellDataPreference) findPreference(KEY_DATA_ENABLE);
            mCellDataPreference.setEnabled(false);
        } else {
            category.setTitle(formatCarrierName(slotId, displayName));
        }
        category.setTemplate(getNetworkTemplate(subId), subId, services);
        category.pushTemplates(services);
    }

    private String formatCarrierName(int slotId, CharSequence displayName) {
        if (!TextUtils.isEmpty(displayName) && !displayName.toString().contains("CARD")) {
                return getContext().getString(R.string.sim_card_number_title, slotId+1)+" - "+displayName;
        }
        return getContext().getString(R.string.sim_card_number_title, slotId+1);
    }
    ...
    public void onResume() {
        ...
        getContext().registerReceiver(mDataSwitchReceiver, intentFilter);
    }

    @Override
    public void onPause() {
        super.onPause();
        getContext().unregisterReceiver(mDataSwitchReceiver);
    }

    private BroadcastReceiver mDataSwitchReceiver = new BroadcastReceiver() {
        @Override
    ...
    private void updateState() {
        ...
        for (int i = 1; i < screen.getPreferenceCount(); i++) {
            TemplatePreferenceCategory preferenceCategory = (TemplatePreferenceCategory) screen.getPreference(i);
            preferenceCategory.pushTemplates(services);
            if (preferenceCategory.getSubId() != 0) {
                updateMobileState(preferenceCategory);
            }
        }
    }

    private void updateMobileState(TemplatePreferenceCategory preferenceCategory) {
        int slotId = SubscriptionManager.getPhoneId(preferenceCategory.getSubId());
        preferenceCategory.setEnabled(getProvisionStatus(slotId));
    }

    private boolean getProvisionStatus(int slotId) {
        int state = INVALID_STATE;
        try {
                IExtTelephony extTelephony =
                        IExtTelephony.Stub.asInterface(ServiceManager.getService("extphone"));
               state = extTelephony.getCurrentUiccCardProvisioningStatus(slotId);
            } catch (RemoteException|NullPointerException ex) {
                state = INVALID_STATE;
                Log.d(TAG, "couldn't connect to extphone service, Exception: "+ ex);
            }
